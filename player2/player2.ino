...
void setup() {
  ...
  randomSeed(analogRead(4));  //pt a merge random corect
  ...
  // Atașăm o funcție care să se declanșeze atunci când primim ceva
...
int onSegment(int px, int py, int qx, int qy, int rx, int ry) {
  if (qx <= getMax(px, rx) && qx >= getMin(px, rx) && qy <= getMax(py, ry) && qy >= getMin(py, ry))
    return 1;  //true
...
int hitContor = 0;
void loop() {
  if (transmis == 0 && generated == 1 && received == 1) {
  ...
  if (WON == -1 && me == 1) {
    ...
    if (WON == 1 && wonFlag == 0) {
      ...
      Wire.beginTransmission(9);  // transmitem spre device #9
      Wire.write(3);              // trimitem 2 sa stie ca e randul lui
      Wire.endTransmission();     // oprim transmisia
      wonFlag = 1;
    }
    if (me == 0) {
